...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['create'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO sta_rosa (name, designation, phone, block_lot, phase, location, date_registration, date_expiration, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        $stmt = $conn->prepare("UPDATE sta_rosa SET name=?, designation=?, phone=?, block_lot=?, phase=?, location=?, date_registration=?, date_expiration=?, status=? WHERE id=?");
$stmt->bind_param("sssssssssi", $name, $designation, $phone, $block_lot, $phase, $location, $date_registration, $date_expiration, $status, $id);
    ...
    $id = $_GET['delete'];
    $stmt = $conn->prepare("DELETE FROM sta_rosa WHERE id=?");
    $stmt->bind_param("i", $id);
    if ($stmt->execute()) {
        header("Location: hoa_members.php?deleted=1");
        exit();
    } else {
    ...
    $stmt->close();
    header("Location: hoa_members.php?success=1");
    exit();
}

// Create table if not exists
$sql = "CREATE TABLE IF NOT EXISTS sta_rosa (
...
// Fetch all records
$result = $conn->query("SELECT * FROM sta_rosa ORDER BY date_expiration ASC");
