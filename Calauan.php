...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['create'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO calauan (name, designation, phone, block_lot, phase, location, date_registration, date_expiration, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        $stmt = $conn->prepare("UPDATE calauan SET name=?, designation=?, phone=?, block_lot=?, phase=?, location=?, date_registration=?, date_expiration=?, status=? WHERE id=?");
$stmt->bind_param("sssssssssi", $name, $designation, $phone, $block_lot, $phase, $location, $date_registration, $date_expiration, $status, $id);
    ...
    $id = $_GET['delete'];
    $stmt = $conn->prepare("DELETE FROM calauan WHERE id=?");
    $stmt->bind_param("i", $id);
    if ($stmt->execute()) {
        header("Location: calauan.php?deleted=1");
        exit();
    } else {
...
$sql = "CREATE TABLE IF NOT EXISTS calauan (
...
// Fetch all records
$result = $conn->query("SELECT * FROM calauan ORDER BY date_expiration ASC");
