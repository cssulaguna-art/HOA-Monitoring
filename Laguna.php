...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['create'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO laguna (name, designation, phone, block_lot, phase, location, date_registration, date_expiration, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        $stmt = $conn->prepare("UPDATE laguna SET name=?, designation=?, phone=?, block_lot=?, phase=?, location=?, date_registration=?, date_expiration=?, status=? WHERE id=?");
$stmt->bind_param("sssssssssi", $name, $designation, $phone, $block_lot, $phase, $location, $date_registration, $date_expiration, $status, $id);
    ...
    $id = $_GET['delete'];
    $stmt = $conn->prepare("DELETE FROM laguna WHERE id=?");
    $stmt->bind_param("i", $id);
    if ($stmt->execute()) {
        header("Location: Laguna.php?deleted=1");
        exit();
    } else {
    ...
    $stmt->close();
    header("Location: Laguna.php?success=1");
    exit();
}

// Create table if not exists
$sql = "CREATE TABLE IF NOT EXISTS laguna (
...
// Fetch all records
$result = $conn->query("SELECT * FROM laguna ORDER BY date_expiration ASC");
...
<body class="bg-gray-100">
  ...
  <div class="container mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white rounded-lg shadow p-6 card">
                ...
                <div class="flex items-center">
                    ...
                    <p class="text-2xl font-bold">
                        <?php 
                            $active = $conn->query("SELECT COUNT(*) FROM homeowners WHERE status='Active' AND date_expiration >= '$current_date'")->fetch_row()[0];
                            echo $active;
                        ?>
                    </p>
                </div>
            ...
            <div class="bg-white rounded-lg shadow p-6 card">
                ...
                <div class="flex items-center">
                    ...
                    <p class="text-2xl font-bold">
                        <?php 
                            $expired = $conn->query("SELECT COUNT(*) FROM homeowners WHERE status='Inactive' OR date_expiration < '$current_date'")->fetch_row()[0];
                            echo $expired;
                        ?>
                    </p>
                </div>
    ...
    ?>
    <tr>
        <td colspan="11" class="px-6 py-4 text-center text-sm text-gray-500">No homeowners registered yet.</td>
    </tr>
    <?php } ?>
</tbody>
                </table>
