...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['create'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO cabuyao (name, designation, phone, block_lot, phase, location, date_registration, date_expiration, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        $stmt = $conn->prepare("UPDATE cabuyao SET name=?, designation=?, phone=?, block_lot=?, phase=?, location=?, date_registration=?, date_expiration=?, status=? WHERE id=?");
$stmt->bind_param("sssssssssi", $name, $designation, $phone, $block_lot, $phase, $location, $date_registration, $date_expiration, $status, $id);
    ...
    $id = $_GET['delete'];
    $stmt = $conn->prepare("DELETE FROM cabuyao WHERE id=?");
    ...
    $stmt->close();
    header("Location: cabuyao.php?success=1");
    exit();
}

// Create table if not exists
$sql = "CREATE TABLE IF NOT EXISTS cabuyao (
...
// Fetch all records
$result = $conn->query("SELECT * FROM cabuyao ORDER BY date_expiration ASC");
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white rounded-lg shadow p-6 card">
                ...
                <div class="flex items-center">
                    ...
                    <p class="text-2xl font-bold">
                        <?php 
                            $active = $conn->query("SELECT COUNT(*) FROM cabuyao WHERE status='Active' AND date_expiration >= '$current_date'")->fetch_row()[0];
                            echo $active;
                        ?>
                    </p>
                </div>
            ...
            <div class="bg-white rounded-lg shadow p-6 card">
                ...
                <div class="flex items-center">
                    ...
                    <p class="text-2xl font-bold">
                        <?php 
                            $expired = $conn->query("SELECT COUNT(*) FROM cabuyao WHERE status='Inactive' OR date_expiration < '$current_date'")->fetch_row()[0];
                            echo $expired;
                        ?>
                    </p>
                </div>
